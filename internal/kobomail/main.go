...
var KoboMailConfig *config.KoboMailConfig
...
func prepareNickelDbusIntegration() error {
	...
	if nickeldbus.IsInstalled() {
		...
		logger.Debug("Did not find NickelDbus")
		useNickelDbus = false
	}
	return nil
}
...
func prepareNickelMenuIntegration() error {
	if nickelmenu.ConfigFileFound() {
		if !nickelmenu.IsInstalled() || !KoboMailConfig.ApplicationConfig.CreateNickelMenuEntry {
			logger.Debug("Unwanted NickelMenu configuration file found, removing")
...
func Run() {
	...
	if err != nil {
		...
		logger.Fatal(errMsg, zap.Error(err))
	}
	logger.Info(
	...
	for _, msg := range messages {
		...
		if err != nil {
			...
			showDialog(errMsg+": "+err.Error(), true)
			logger.Fatal(errMsg, zap.Any("message", msg), zap.Error(err))
		}
		logger.Info("Processing message", zap.Any("message", msg))

		downloadedAttachmentsCount, err := msg.ProcessAttachments(KoboMailConfig.ProcessingConfig.Filetypes, config.New().ApplicationConfig.LibraryPath)
		if err != nil {
